...
Cylindre::Cylindre(int nVertice)
    : TriMesh()
{
    _name = "Cylindre";
    int i;
    ...
    for (j=0; j<nLayer; j++) //side vertex
    {
        z = -1 + (2.0/(nLayer-1))*j;
    ...
    for (i=nVertice*(nLayer-1); i<nVertice*nLayer; i++)
    {
        if ((i+1) % nVertice == 0)
        {
            this->addTriangle(nVertice*nLayer+1, i+1, i+2-nVertice);
        }
        else
        {
            this->addTriangle(nVertice*nLayer+1, i+1, i+2);
        }
    }

    //fill triangle for the side
    for (i=0; i<nVertice*(nLayer-1); i++)
    {
        if ((i+1) % nVertice == 0) // example if i=19
        {
